...
\begin{document}
\maketitle
\textbf{Manuscript:} Burden-Constrained Policy-Layer Authorization for Simulated EEG-Guided Adaptive Vagus Nerve Stimulation.

We thank the reviewers for comments that prompted a more complete and rigorous evaluation of the proposed framework. The revised manuscript now includes complete-recording held-out evaluation, implemented temporal comparator rules, a common-score factorial analysis, attribution-lag sensitivity, causal decision-time accounting, targeted robustness diagnostics, and executable reproducibility materials.

As part of this revision, we completed the recording inventory for all five cases and extended the held-out evaluation to all available recordings. The broader evaluation yielded a higher principal false-cluster estimate than the original partial inventory, providing a more conservative basis for assessing operational burden. We report the updated result throughout the manuscript and have correspondingly tightened the scope of the claims. Two final targeted analyses further tested the remaining methodological flanks: reclassifying cluster truth at window-end decision availability left the principal burden endpoint unchanged, and EDF-level decomposition showed that the high chb05 burden was distributed across recordings rather than driven by one or two anomalous files. The complete executable analysis accompanies the revision and is prepared for a new versioned repository release.
...
\textbf{Response.} We expanded the Introduction and retained a structured methodological table and a visually neutral coverage map, while also adding an implemented common-score comparator benchmark rather than relying on description alone. The benchmark applies causal majority voting and firing-power mechanisms to the same calibrated LightGBM outputs, original calibration recordings, and complete held-out EEG as thresholding, persistence, and persistence plus attribution overlap. Majority voting uses the current and two preceding binary decisions; firing power averages a strictly past/current decision history and applies a 0.7 cutoff. The selected mechanisms are motivated by the cited temporal-processing studies, and we explicitly identify them as adaptations: we did not reproduce the external studies' complete artifact detectors, classifiers, training datasets, or hardware implementations.
...
The benchmark shows overlapping operational trade-offs rather than a uniformly superior configuration. With 300 s refractory suppression, persistence plus overlap produced $0.611\pm1.140$ false clusters/h and covered 17/27 events, versus $0.614\pm1.193$/h and 16/27 for majority voting, and $0.517\pm1.007$/h and 15/27 for firing power. None of those refractory benchmark configurations authorized a seizure pre-onset. The paired complete-exposure burden differences relative to persistence plus overlap were 0.003/h for majority voting (exploratory 95\% case-bootstrap interval $-0.051$ to 0.062) and $-0.094$/h for firing power ($-0.267$ to 0.083); descriptive sign-flip probabilities were 1.000 and 0.313, respectively. These five-case summaries are explicitly treated as exploratory rather than confirmatory inference. Without suppression, firing power attained 12 pre-onset authorizations, but at settings authorizing over 91\% of windows in two cases. This illustrates why cluster counts should be interpreted together with authorization occupancy when assessing operational burden.

The revised table and figures show these trade-offs, and the paired values, all training-search candidates, and executable implementations are included in Supplementary Software 1. The Discussion states both the methodological contribution of explicit policy-layer evaluation and the scope limitations of the present retrospective five-case, non-device evaluation. The methodological coverage map has also been reformatted to avoid a filled-cell visual ranking; its symbols indicate only whether a stage was explicitly modeled or evaluated. The last column is explicitly labeled simulated VNS authorization, not a generic action policy.

\textbf{Locations in revised manuscript.} Introduction; Table 1; Figure 1; Sections 2.9--2.10; Section 3.5; Table 8; Figure 5; Supplementary paired-comparator table.
...
\textbf{Response.} We traced the collection back to the original loader and acquisition log. Before the collection audit, only 70 EDFs were locally available to the analysis pipeline: all 27 seizure-containing recordings in the five configured cases and 43 of their 148 EDFs without annotated seizures. The remaining 105 EDFs had not been omitted on the basis of a documented quality criterion. We therefore completed the collection and revised the analysis account around the full set of available recordings.

We then completed the inventory by acquiring and evaluating the remaining 105 recordings. All 175 available EDFs in these cases passed the preparation checks; no added recording was excluded. The complete exposure is 172.8294 h and 310,916 windows, compared with 67.8214 h and 122,007 windows before recording completion. The positive-label count and seizure count remain 5,227 and 27. The restored recordings were evaluation-only: the predefined core, calibration, and development-test membership of the fitted pipeline was preserved, and no restored EDF entered fitting or threshold selection. The acquisition checks and feature-reference comparisons are included in Supplementary Software 1.

The within-subject cutoffs are now given explicitly. The appendix reports both within-subject and held-out core/calibration/test counts, hours, positive windows, and seizure-containing recordings. Supplementary Software 1 includes exact EDF-level membership in addition to aggregate counts.

The available development record does not document a prospective clinical selection rule for the five cases or establish that case selection preceded every stage of model and policy development. The revised manuscript therefore describes these cases conservatively as a convenience subset and makes no claim of prospective or preregistered selection. Completing the recording inventory resolves the within-case exposure issue but does not add independent patients or seizures.

The complete-recording result is retained transparently: the historical nominal held-out full-governed configuration was $1.17\pm2.05$ false clusters/h on the original exposure, whereas the train-selected persistence-plus-overlap policy on complete exposure produced $3.60\pm7.75$/h, with 681 of 700 false clusters in chb05. We additionally decomposed the chb05 result by EDF. False clusters occurred in 32/39 chb05 recordings; the largest EDF contributed only 12.6\% of the total, the three largest 33.5\%, and the five largest 48.3\%. The 25 subsequently restored evaluation-only EDFs contributed 615 false clusters over 25 h (24.6/h), with false clusters in 22/25 recordings. Thus, the chb05 result is distributed across held-out recordings rather than being attributable to a single anomalous EDF.

\textbf{Locations in revised manuscript.} Sections 2.2--2.3 and 2.11; Table 2; Sections 3.3 and 3.7; Table 6; Figure 3; Supplementary Tables 9--10 and targeted robustness diagnostics.
...
\textbf{Response.} The Methods now gives the exact implemented support-based endpoint. Within an EDF, selected windows merge when the next selected support starts at or before the current cluster end plus the cooldown. With four-second windows and a two-second step, selected supports can touch despite an intervening unselected decision; the revised Methods therefore clarifies that clusters are not necessarily uninterrupted runs of positive decisions.

A cluster is false only when none of its selected windows has a positive operational label. A boundary-crossing cluster with at least one selected positive window is non-false. Envelope overlap or an unselected positive window alone does not change its classification. Overlapping event intervals form a union for labeling, while event coverage is assessed separately per seizure. Clusters never cross EDF boundaries.

The primary denominator is full analyzed duration of unique EDFs in the specified exposure, including preictal and ictal time. We call this false clusters per analyzed recording-hour, not an interictal-only false-alarm rate. A supplementary rate with the positive-interval union removed from the denominator is also provided. Selected-support occupancy is reported separately so that long continuous positive states are not interpreted as low burden solely because they form few clusters.
...
We also performed the final decision-time sensitivity requested by the causal timing revision. Holding authorization masks and cluster construction fixed, we reclassified cluster truth using operational labels evaluated at window-end decision availability rather than window centers. For the principal persistence-plus-overlap policy, no cluster changed classification in any case: the false-cluster counts remained 2, 12, 4, 681, and 1, totaling 700 (4.050/h). Across the full factorial set, only one single-window chb05 cluster changed under thresholding alone and overlap alone; neither persistence-based configuration changed. The principal burden conclusion is therefore insensitive to this timing convention.

\textbf{Locations in revised manuscript.} Sections 2.7, 2.11, 3.3, and 3.7; Table 6; Supplementary Table 13 and targeted decision-time sensitivity table.
...
\textbf{Response.} We revised this analysis so that the original rows are identified as a nominal configuration comparison rather than an isolating ablation, and added a factorial analysis in which components are varied independently on a common calibrated probability stream. The original configurations remain identifiable in the appendix, including their different probability inputs.
...
The previously highlighted star is now identified as the nominal setting $\theta=0.5$, $\gamma=0.4$, rather than an optimized operating point. The corrected sweep keeps $\gamma$ fixed. No operating point is selected from the added test recordings, and no matched-test-burden or superiority claim is made.
...
\textbf{Response.} The manuscript now states before the Results that the binary operational target combines preictal and ictal windows and is therefore evaluated as seizure-responsive authorization rather than assumed to be an advance seizure-prediction target. Pre-onset authorization is treated as a separate event-timing outcome from the outset.

The abstract reports the principal train-selected policy's full event categories and latency, not only broad coverage: 2/27 pre-onset, 18/27 at-or-after-onset only, and 7/27 missed. Pre-onset coverage is 7.4\% of all seizures and 10\% of the 20 covered seizures. Pooled coverage is 20/27 (74.1\%), distinct from equal-weight fold coverage of 78.9\%.

We also revised the timing clock to reflect causal availability. A four-second feature window cannot authorize intervention at its center because half its samples are not yet available at that time. The revised principal timing therefore uses window ends, without introducing an assumed processing or device delay. Median latency among covered events is 16 s, with interquartile range 8.5--28 s; the center-referenced median was 14 s. The principal event-category counts remain unchanged, but they were recalculated from the complete mask under the revised timing definition.
...
The Discussion and Conclusion therefore distinguish seizure-responsive authorization from advance prediction and therapeutic validation. Post-onset authorization is not treated as evidence of seizure termination or delivered-VNS benefit.

\textbf{Locations in revised manuscript.} Introduction; Sections 2.2, 2.8, and 2.11; Sections 3.5 and 3.7; Figure 5A; Supplementary Table 14.
...
\textbf{Response.} The operating figure has been regenerated. During regeneration, we identified two implementation/reporting mismatches and revised the analysis accordingly: the earlier curve averaged over different attribution thresholds, and the former middle axis represented the fraction of all windows authorized rather than positive authorization rate. The revised sweep fixes $\gamma=0.4$, varies $\theta$ over 0.1--0.9 in steps of 0.1, and plots equal-weight case means for one defined configuration at each point. The nominal star is $\theta=0.5$, $\gamma=0.4$; it is not an optimized point.
